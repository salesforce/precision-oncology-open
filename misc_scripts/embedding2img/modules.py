...
class EmbeddingsProjector(object):
    ...
    def calculate_projection(self):
        ...
        if self.method == SKLEARN:
            ...
            import umap
            projection_vectors = umap.umap_.UMAP(n_neighbors=5, min_dist=0.3).fit_transform(self.data_vectors)
    ...
    def _scatter(self):
        ...
        for i in tqdm(range(image_num)):
            ...
            if self.colors is not None:
                y_len = (y0 + dy + y1 + 40) - (y0 + dy - 40)
                x_len = (x0  + dx + x1 + 40) - (x0 + dx - 40)
                color_box = np.ones((y_len, x_len, 3)) * self.colors[self.cluster_labels[i]]
                try:
                    image[y0 + dy - 40 :y0 + dy + y1 + 40, x0 + dx - 40:x0  + dx + x1 + 40] = color_box
                except:
                    print("failed at: ", x0, y0)
            image[y0 + dy:y0 + dy + y1, x0 + dx:x0 + dx + x1] = small_img
