...
class CustomDataset(VisionDataset):
    ...
    def __getitem__(self, index):
        ...
        if type(feature_quilt_path) == str and os.path.isfile(feature_quilt_path):
            ...
            feature_quilt = torch.zeros((128, 200, 200)).double()
...
for n in categorical_names:
    ...
    X_train[n] = label_encoder.transform(X_train[n])
    X_test[n] = label_encoder.transform(X_test[n])
...
for n in continuous_names:
    ...
    if len(new_vals) > 0:
        X[n] = new_vals
        X_train[n] = imp.transform(X_train[n].values.reshape(-1,1)).reshape(-1,)
        X_test[n] = imp.transform(X_test[n].values.reshape(-1,1)).reshape(-1,)
    else:
...
for n in continuous_names:
    ...
    X_train[n] = scaler.transform(X_train[n].values.reshape(-1,1))
    X_test[n] = scaler.transform(X_test[n].values.reshape(-1,1))
