...
parser = argparse.ArgumentParser(
    description='NRG Feature-Block Outcome Finetuning.')
parser.add_argument('--pod', required=True, type=str,
                    help='Option --pod missing. E.g. --pod=2')
args = parser.parse_args()

N = 10 # Number of runs to execute
out_dir = "/export/medical_ai/ucsf/ssl_rtog/moco/model_R50_b=256_lr=0.03_pg4plus_imagenet_pretrained/distant_met_5year/image_training/no_lr_schedule/pod{}/{}"
out_dir = out_dir.format(args.pod, {})
command = """
python train_rtog_cnn_on_representations_outcome_testset.py \
  --optimizer=Adam \
  --num_epochs=300 \
  --early_stopping=50 \
  --weight_decay=5e-5 \
...
for i in range(N):
    lr = 0.1 ** ((4 + np.random.uniform() * 4)) # sample between 1e-4 and 1e-8
